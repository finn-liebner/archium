...
# archium (v0.1) - a script to install Arch Linux
...
echo " /#                                 #\ "
echo -e "\e[0m\e[1m"
echo " <<<    Arch Linux Shell-Skript    >>> "
echo " <<<    Finn Liebner 24/01/2021    >>> "
echo -e "\e[0m"

start=$(date +%s)

# Einlesen der Speichergeräte, Anzahl ermitteln
blk=$(lsblk -l -i -o NAME -d | sed '1d')
num=$(echo "$blk" | grep -c '')

if [ $num -eq 0 ]; then
  echo -e "\e[1;31mFehler - keine Speichergeräte\e[0m"
elif [ $num -eq 1 ]; then
  dev=$blk
else
  echo -e "\e[1;32mAuswählen:\e[0m"
  echo "$(lsblk)"
  echo -e "\e[1;32m"
  read input
  echo -e "\e[0m"
...
# uefi (EF00: EFI-System / 8304: Linux root)
# (echo n; echo 1; echo 2048; echo +512M; echo EF00; echo n; echo 2; echo ; echo ; echo 8304; echo w; echo Y) | gdisk /dev/$dev > /dev/null
...
# Partition 1 formatieren, nur für uefi
# mkfs.fat -F 32 -n EFIBOOT /dev/$pt1

# Partition 2 formatieren, bei bios und uefi
mkfs.ext4 -L arch /dev/$pt2

# Wurzelverzeichnis einhängen
mount -L arch /mnt
mkdir -p /mnt/boot

# Bootpartition einhängen
# mount -L EFIBOOT /mnt/boot

################################################################################

# Funktion zum Zusammenstellen der Pakete
function pkglist()
{
  local return='sudo networkmanager network-manager-applet pulseaudio mesa wayland gdm mutter papirus-icon-theme arc-solid-gtk-theme nautilus gvfs-mtp gvfs-nfs gnome-{control-center,session,settings-daemon,shell,shell-extensions,terminal,screenshot} firefox firefox-i18n-de firefox-ublock-origin atom neofetch'
  echo "$return"
}
...
pacman --noconfirm --needed --root /mnt -S netctl dhcpcd dialog
...
cat << EOF | arch-chroot /mnt/
  ...
  pacman --noconfirm --needed -S networkmanager network-manager-applet mesa xf86-video-intel vulkan-intel wayland gdm mutter papirus-icon-theme arc-solid-gtk-theme nautilus gvfs-mtp gvfs-nfs gnome-{control-center,session,settings-daemon,shell,shell-extensions,tweaks,tweak-tool}
...
cat << EOF | arch-chroot /mnt/
  pacman --noconfirm --needed -S $(pkglist)
EOF
...
time=$(date -ud "@$((end-start))" +"%Mmin %Ssek")
echo -e "Installation abgeschlossen. Laufzeit ${time}\n"
echo -e " \"umount -R /mnt && reboot\" zum Neustarten.\n"
